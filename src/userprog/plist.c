...
void plist_remove(struct plist* m, key_p id) //is key right??? 
{
    ...
    */
	...
	if(m->content[i].parent_id == id) //sets the children to have no parent alive
	{
	    m->content[i].parent_alive = false;
...
void plist_print(const struct plist* m) 
{
    ...
    for(int i = 0; i < PLISTMAP_SIZE; i++)
	if (!(m->content[i].free))
	    printf("id: %i, parent_id: %i, exit_status: %i, parent_alive: %d \n", m->content[i].proc_id, m->content[i].parent_id, m->content[i].exit_status, m->content[i].parent_alive);
